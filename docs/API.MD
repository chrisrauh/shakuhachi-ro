...
export default {
  setup(props) {
    ...
    onMounted(async () => {
      ...
      try {
        await renderer.renderFromScoreData(await MusicXMLParser.parseFromURL(props.scoreUrl));
      } catch (error) {
...
class ScoreWidget {
  ...
  async loadScore(url: string): Promise<void> {
    try {
      await this.renderer.renderFromScoreData(await MusicXMLParser.parseFromURL(url));
    } catch (error) {
