/**
 * Layout Tests for Score Detail Page
 *
...
test.describe('Score Detail Page - Layout', () => {
  test('score renderer fills full viewport height', async ({ page }) => {
    ...
    const expectedMinHeight =
      viewportHeight - (headerHeight?.height || 0) - 100; // 100px buffer for spacing
  ...
  test('score detail container has full-height flex layout', async ({
    page,
  }) => {
    await page.goto('/score/akatombo');
    await page.waitForLoadState('networkidle');
    ...
    const containerStyles = await page
      .locator('.score-detail-container')
      .evaluate((el) => {
        const computed = window.getComputedStyle(el);
        return {
  ...
  test('body has full-height class when fullHeight enabled', async ({
    page,
  }) => {
    await page.goto('/score/akatombo');
    await page.waitForLoadState('networkidle');
  ...
  test('score renderer fills viewport on desktop (1280x720)', async ({
    page,
  }) => {
    ...
    expect(rendererBox).not.toBeNull();
    const expectedMinHeight =
      viewportHeight - (headerHeight?.height || 0) - 100;
    expect(rendererBox!.height).toBeGreaterThan(expectedMinHeight);
  });

  test('score renderer fills viewport on tablet (768x1024)', async ({
    page,
  }) => {
  ...
  test('score renderer fills viewport on mobile (375x667)', async ({
    page,
  }) => {
