...
- Score pages are edge-cached and therefore shared between viewers — the SSR HTML must stay identical for everyone. Anything that makes it depend on who is asking (private scores, reading cookies or an `Authorization` header during SSR) needs the caching in `src/pages/score/[slug].astro` revisited first. See [docs/ARCHITECTURE-PLATFORM.MD](./docs/ARCHITECTURE-PLATFORM.MD)
